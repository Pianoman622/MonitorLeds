...
void DrawPixelsNew(float fPos, float count, CRGB color, CRGB * jStrip)
{
  ...
  if (remaining > 0.0f)
  {
    jStrip[iPos++] += ColorFraction(color, amtFirstPixel);
  ...
  if (remaining > 0.0f)
  {
    jStrip[iPos] += ColorFraction(color, remaining);
  }
}
