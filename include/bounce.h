...
class BouncingBallEffect
{
  private:
    ...
    const double SpeedKnob = 2;                   // Higher values will slow the effect
  ...
  public:
    ...
    virtual void Draw(CRGB * jStrip)
    {
        if (_fadeRate != 0)
        {
            for (size_t i = 0; i < _cLength; i++)
                jStrip[i].fadeToBlackBy(_fadeRate);
        }
        else
            FastLED.clear();
        ...
        for (size_t i = 0; i < _cBalls; i++)
        {
            ...
            DrawPixelsNew(position, 1, Colors[i], jStrip);
            DrawPixelsNew(position+1, 1, Colors[i], jStrip);

            if (_bMirrored)
            {

                DrawPixelsNew(_cLength - 1 - position, 1, Colors[i], jStrip);
                DrawPixelsNew(_cLength - position, 1, Colors[i], jStrip);

                // conveyorLeds[_cLength - 1 - position] += Colors[i];
                // conveyorLeds[_cLength - position]     += Colors[i];
            }
        }
        //delay(20);
    }
