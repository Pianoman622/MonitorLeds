...
class FireEffectSmooth
{
  ...
  public:
    ...
    void DrawFire(CRGB * jStrip)
    {
        //fill_solid(jStrip, Size, CRGB::Black);
        //FastLED.clear();
        ...
        for (int i = 0; i < Size; i++)
        {
            DrawPixelsNew(i, 1, HeatColor(240 * min(1.0f, Temperatures[i])), jStrip);
...
public:
    ...
    virtual void DrawFire(CRGB * jStrip)
    {
        ...
        for (int i = 0; i < Size; i++)
        {
            ...
            jStrip[j]= color;
            //DrawPixelsNew(j, 1, color, jStrip);
            if (bMirrored)
            {
                int j2 = !bReversed ? (2 * Size - 1 - i) : Size + i;
                jStrip[j2] = color;
                //DrawPixelsNew(j2, 1, color, jStrip);
            }
        }
    delay(10);
    }
